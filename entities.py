...
class entity(object):
    ...
    def get_current_img(self):
        if self.animation == None:
            ...
            return flip(
                animation_database[self.animation[self.animation_frame]], self.flip
            )
    ...
    def display(self, surface, scroll):
        if self.animation == None:
            if self.image != None:
                surface.blit(
                    flip(self.image, self.flip),
                    (
                        int(self.x) - scroll[0] + self.offset[0],
                        int(self.y) - scroll[1] + self.offset[1],
                    ),
                )
        else:
            surface.blit(
