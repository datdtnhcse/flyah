...
player_walking_anim = e.animation_sequence(
    [[0, 4], [1, 4], [2, 4], [3, 4], [4, 4], [5, 4]], "data/images/player/walking_"
)
jump_anim = e.animation_sequence(
    [[0, 3], [1, 3], [2, 3], [3, 3], [4, 3], [5, 3], [6, 3]],
    "data/images/jump_particles/jump_",
)
...
slime_images = [
    load_img("slime/slime_0"),
    load_img("slime/slime_1"),
    load_img("slime/slime_2"),
]
...
grass_sfx = [
    pygame.mixer.Sound("data/sfx/grass_0.wav"),
    pygame.mixer.Sound("data/sfx/grass_1.wav"),
    pygame.mixer.Sound("data/sfx/grass_2.wav"),
]
...
true_offset = [0, 0]
hitboxes = [
    pygame.Rect(12, 103, 92, 21),
    pygame.Rect(12, 124, 70, 60),
    pygame.Rect(127, 173, 147, 100),
    pygame.Rect(304, 90, 82, 100),
]
...
while True:
    ...
    for cloud in clouds:
        display.blit(
            cloud_images[cloud[3]],
            (cloud[1] - offset[0] * cloud[0], cloud[2] - offset[1] * cloud[0]),
        )
    # Island Decor ------------------------------------------- #
                ...
                for i in range(8):
                    slime_balls.append(
                        [
                            ...
                            random.randint(0, 45),
                            [
                                random.randint(1, 100) / 50 - 1,
                                random.randint(1, 100) / 200 - 0.75,
                            ],
                            "smoke",
                        ]
                    )
            ...
            if player_movement[0] > 0:
                particles.append(
                    [
                        ...
                        random.choice([(161, 239, 121), (63, 199, 120), (0, 163, 131)]),
                    ]
                )
            elif player_movement[0] < 0:
                particles.append(
    ...
    if player_flash_timer > 0:
        ...
        if random.randint(1, 2) == 1:
            blit_center(
                display,
                pygame.transform.rotate(gun_barrel_img, gun_angle),
                (player.x + 6 - offset[0], player.y + 5 - offset[1]),
            )
    ...
    if random.randint(0, slime_rate) == 0:
        pos = random.choice(
            [
                [-24, random.randint(0, 50)],
                [random.randint(0, 400), -40],
                [400, random.randint(0, 50)],
            ]
        )
        pick = None
        for i in range(100):
    ...
    for slime in slimes:
        ...
        if dirts[slime[4]][0] != "null":
            if slime_entity.obj.rect.colliderect(
                pygame.Rect(dirts[slime[4]][0], dirts[slime[4]][1], 10, 8)
            ):
                slime[1] = [0, 0]
                slime[2] = 0
                dirts[slime[4]][3] -= 2
        ...
        blit_center(
            ...
            pygame.transform.rotate(
                pygame.transform.scale(
                    slime_img,
                    (
                        24 - int(slime_speed * 2),
                        34 + int(slime_speed * 3) - int(slime[5][0] / 5),
                    ),
                ),
                slime[2],
            ),
            (slime[0][0] - offset[0] + 2, slime[0][1] - offset[1] + 8),
        )
        ...
        if slime[3][0] != slime[3][1]:
            display.blit(
                health_bar, (slime[0][0] - offset[0] - 8, slime[0][1] - offset[1] - 4)
            )
            if slime[3][0] > 0:
                ...
                display.blit(
                    health_surf,
                    (slime[0][0] - offset[0] - 7, slime[0][1] - offset[1] - 3),
                )
        if slime[3][0] <= 0:
            slimes.remove(slime)
            for i in range(30):
                ...
                slime_balls.append(
                    [
                        slime[0][0] + random.randint(0, 24) - 4,
                        slime[0][1] + random.randint(0, 24) - 8,
                        random.randint(5, 15),
                        random.randint(0, 360),
                        random.randint(0, 90) - 45,
                        [
                            random.randint(1, 100) / 50 - 1,
                            random.randint(1, 100) / 50 - 1,
                        ],
    ...
    for particle in particles:
        ...
        if particle[3] > 2:
            particle[3] = 2
        display.set_at(
            (int(particle[0]) - offset[0], int(particle[1]) - offset[1]), particle[5]
        )
        particle[4] -= 1
        if particle[4] <= 0:
    ...
    for ball in slime_balls:
        ...
        if len(ball) == 6:
            blit_center(
                display,
                pygame.transform.rotate(
                    pygame.transform.scale(slime_ball, (int(ball[2]), int(ball[2]))),
                    ball[3],
                ),
                (ball[0] - offset[0], ball[1] - offset[1]),
            )
        else:
            blit_center(
                display,
                pygame.transform.rotate(
                    pygame.transform.scale(smoke_img, (int(ball[2]), int(ball[2]))),
                    ball[3],
                ),
                (ball[0] - offset[0], ball[1] - offset[1]),
            )
    ...
    for bullet in bullets:
        ...
        if (bullet[4] == "planted") or (bullet[3] >= 3):
            blit_center(
                display,
                pygame.transform.rotate(bullet_img, -bullet[2]),
                (int(bullet[0]) - offset[0], int(bullet[1]) - offset[1]),
            )
    ...
    if gun_target != None:
        ...
        if target_timer < 30:
            display.blit(
                target_img,
                (target_pos[0] - 6 - offset[0], target_pos[1] - 6 - offset[1]),
            )
        else:
            display.blit(
                target_2_img,
    ...
    for alert in alerts:
        ...
        display.blit(
            text_surf, (alert[1] - int(width / 2) - offset[0], alert[2] - offset[1])
        )
        alert[3] += 1
        if alert[3] == 90:
            alerts.remove(alert)
    ...
    if firing == True:
        ...
        if fire_timer >= 7:
            ...
            if ammo > 0:
                bullets.append(
                    [player.x + 6, player.y + 5, -(gun_angle + 90), 0, gun_mode]
                )
                ammo -= 1
                pew_sfx.play()
    else:
        ...
        if event.type == KEYDOWN:
            ...
            if event.key == K_UP:
                if jumps > 0:
                    player_momentum[1] = -5
                    jumps -= 1
                    jump_particles.append(
                        [e.entity(player.x - 2, player.y + 7, 2, 2), 0]
                    )
            if event.key == K_c:
                if gun_mode == "plant":
                    gun_mode = "kill"
    ...
    while lost:
        ...
        if lost_time > 33:
            text.show_text(
                "The hungry slimes ate all of your dirt",
                110,
                50,
                1,
                99999,
            ...
            text.show_text(
                "and you've run out of space to grow bullets.",
                110,
                60,
                1,
                99999,
            ...
            text.show_text(
                "You've lost... Press any key to restart.",
                110,
                70,
                1,
                99999,
            ...
            text.show_text(
                "You lasted " + str(int(game_timer / 60)) + " seconds.",
                110,
                90,
                1,
                99999,
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                ...
                if lost_time > 90:
                    ...
                    air_time = 0
        screen.blit(
            pygame.transform.scale(display, (WINDOWWIDTH, WINDOWHEIGHT)), (0, 0)
        )
        pygame.display.update()
        mainClock.tick(60)
